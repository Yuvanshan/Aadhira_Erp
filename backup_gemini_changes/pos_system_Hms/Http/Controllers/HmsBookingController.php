...
class HmsBookingController extends Controller
{
    ...
    public function booking_room_edit(Request $request)
    {
        ...
        if ($request->has('arrival_date') && !empty($request->arrival_date)) {
            ...
            $exclude_room_ids = HmsRoom::get_booked_and_unavailable_room_ids(
                $type_id,
                $arrival_date_time,
                $departure_date_time,
                $existing_rooms,
        ...
        $rooms = HmsRoom::where('hms_room_type_id', $type_id)->get();

        return view('hms::bookings.edit_room', compact('types', 'type', 'rooms', 'exclude_room_ids', 'room_id', 'no_of_child', 'no_of_adult'));
    ...
    {
        \Log::info('get_room_type_by called: ' . json_encode($request->all()));
        $type_id = $request->input('type_id');
        if (empty($type_id)) {
            return '';
        }
        ...
        try {
            $arrival_date_time = !empty($request->arrival_date) ? ($this->commonUtil->uf_date($request->arrival_date) . ' ' . $this->commonUtil->uf_time($request->arrival_time)) : date('Y-m-d H:i:s');
        } catch (\Exception $e) {
            $arrival_date_time = date('Y-m-d H:i:s');
        }

        try {
            $departure_date_time = !empty($request->departure_date) ? ($this->commonUtil->uf_date($request->departure_date) . ' ' . $this->commonUtil->uf_time($request->departure_time)) : date('Y-m-d H:i:s', strtotime('+1 day'));
        } catch (\Exception $e) {
            $departure_date_time = date('Y-m-d H:i:s', strtotime('+1 day'));
        }

        try {
            $arrival_date = !empty($request->arrival_date) ? $this->commonUtil->uf_date($request->arrival_date) : date('Y-m-d');
        } catch (\Exception $e) {
            $arrival_date = date('Y-m-d');
        }

        try {
            $departure_date = !empty($request->departure_date) ? $this->commonUtil->uf_date($request->departure_date) : date('Y-m-d', strtotime('+1 day'));
        } catch (\Exception $e) {
        ...
        return view('hms::bookings.room_type_by', compact('rooms', 'exclude_room_ids', 'type'));
    }
    ...
    public function print($id)
    {
        ...
        $format = request()->get('format', 'a5');
        $format = strtolower($format) == 'a4' ? 'A4' : 'A5';

        $html = view('hms::bookings.print_pdf')->with(compact('business', 'transaction', 'booking_rooms', 'extras_id', 'extras', 'format'))->render();
        
        // Append script to trigger browser native printing dialog and auto-close
