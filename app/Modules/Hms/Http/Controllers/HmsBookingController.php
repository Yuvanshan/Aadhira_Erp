...
class HmsBookingController extends Controller
{
    ...
    {
        \Log::info('get_room_type_by called (copy): ' . json_encode($request->all()));
        $type_id = $request->input('type_id');
        if (empty($type_id)) {
            return '';
        }
    ...
    public function print($id)
    {
        ...
        $format = strtolower($format) == 'a5' ? 'A5' : 'A4';

        $html = view('hms::bookings.print_pdf')->with(compact('business', 'transaction', 'booking_rooms', 'extras_id', 'extras', 'format'))->render();
        
        $margin_top = $format == 'A5' ? 12 : 16;
        $margin_bottom = $format == 'A5' ? 12 : 16;
        $margin_left = $format == 'A5' ? 14 : 18;
        $margin_right = $format == 'A5' ? 14 : 18;
