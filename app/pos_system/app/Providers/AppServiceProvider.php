...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        View::composer(
            ['*'],
            function ($view) {
                static $shared_sidebar_data;

                if ($shared_sidebar_data === null) {
                    $shared_sidebar_data = [
        ...
        View::composer(
            ['layouts.*'],
            function ($view) {
                static $layout_view_data;

                if ($layout_view_data === null) {
                    $layout_view_data = [];

                    if (isAppInstalled()) {
                        $keys = ['additional_js', 'additional_css'];
                        $__system_settings = Cache::remember('layout_system_settings', 3600, function () use ($keys) {
                            return System::getProperties($keys, true);
                        });

                        //Get js,css from modules
                        $module_additional_script = Cache::remember('layout_module_additional_script', 3600, function () {
                            $moduleUtil = new ModuleUtil;

                            return $moduleUtil->getModuleData('get_additional_script');
                        });
                        $additional_views = [];
                        $additional_html = '';
                        ...
                        $layout_view_data = [
                            '__additional_views' => $additional_views,
                            '__additional_html' => $additional_html,
                            '__system_settings' => $__system_settings,
                        ];
                    }
                }

                if (! empty($layout_view_data)) {
                    $view->with($layout_view_data);
                }
            }
        );
