...
class Util
{
    ...
    public function getCronJobCommand()
    {
        ...
        if (function_exists('exec')) {
            if ($isWindows) {
                $php_binary_path = trim(exec('where php'));
            } else {
                $php_binary_path = trim(exec('which php'));
            }
        } else {
        ...
        if ($isWindows) {
            $command = 'schtasks /Create /SC MINUTE /MO 1 /TN "AadhiraERPBackup" /TR "\"'.$php_binary_path.'\" \"'.$artisan_path.'\" schedule:run" /F';
        } else {
            $command = '* * * * * '.$php_binary_path.' '.$artisan_path.' schedule:run >> /dev/null 2>&1';
        }
