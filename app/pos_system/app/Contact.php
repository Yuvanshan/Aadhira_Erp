...
class Contact extends Authenticatable
{
    ...
    public function scopeOnlySuppliers($query)
    {
        ...
        // Add caching for supplier query
        $cacheKey = 'contacts_suppliers_' . md5(json_encode([$query->toSql(), $query->getBindings()]));
        $cachedResult = \Cache::get($cacheKey);
        if ($cachedResult) {
            return collect($cachedResult);
        }
        $result = $query->whereIn('contacts.type', ['supplier', 'both'])->get();
