...
class ReportController extends Controller
{
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        //Return the details in ajax call
        if ($request->ajax()) {
            $payment_totals = $this->getTransactionPaymentTotalsSubquery();
            $contacts = Contact::where('contacts.business_id', $business_id)
                ->join('transactions AS t', 'contacts.id', '=', 't.contact_id')
                ->leftJoinSub($payment_totals, 'tp_agg', function ($join) {
                    $join->on('t.id', '=', 'tp_agg.transaction_id');
                })
                ->active()
                ->groupBy('contacts.id')
                ->select(
                    ...
                    DB::raw("SUM(IF(t.type = 'purchase', COALESCE(tp_agg.total_amount, 0), 0)) as purchase_paid"),
                    DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final', COALESCE(tp_agg.net_amount, 0), 0)) as invoice_received"),
                    DB::raw("SUM(IF(t.type = 'sell_return', COALESCE(tp_agg.total_amount, 0), 0)) as sell_return_paid"),
                    DB::raw("SUM(IF(t.type = 'purchase_return', COALESCE(tp_agg.total_amount, 0), 0)) as purchase_return_received"),
                    DB::raw("SUM(IF(t.type = 'sell_return', final_total, 0)) as total_sell_return"),
                    DB::raw("SUM(IF(t.type = 'opening_balance', final_total, 0)) as opening_balance"),
                    DB::raw("SUM(IF(t.type = 'opening_balance', COALESCE(tp_agg.net_amount, 0), 0)) as opening_balance_paid"),
    ...
    protected function getTransactionPaymentTotalsSubquery()
    {
