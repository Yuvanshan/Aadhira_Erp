...
Route::middleware('auth:api', 'timezone')->prefix('connector/api')->as('connector.api.')->group(function () {
    Route::resource('business-location', Modules\Connector\Http\Controllers\Api\BusinessLocationController::class)->only('index', 'show');

    Route::resource('booking', Modules\Connector\Http\Controllers\Api\BookingController::class)->only('index', 'show');

    Route::resource('room', Modules\Connector\Http\Controllers\Api\RoomController::class)->only('index');
...
Route::middleware('auth:api', 'timezone')->prefix('connector/api/crm')->as('connector.api.crm.')->group(function () {
...
Route::middleware('auth:api', 'timezone')->prefix('connector/api')->as('connector.api.fieldforce.')->group(function () {
...
Route::get('connector/api/client-mode', function () {
    $business_id = request()->input('business_id');
    if (!empty($business_id)) {
        $business = \App\Business::find($business_id);
        if ($business && !empty($business->settings)) {
            $settings = json_decode($business->settings, true);
            if (!empty($settings['desktop_client_mode'])) {
                return response()->json([
                    'desktop_client_mode' => $settings['desktop_client_mode']
                ]);
            }
        }
    }
    $desktop_mode = \App\System::where('key', 'desktop_client_mode')->value('value');
    return response()->json([
        'desktop_client_mode' => !empty($desktop_mode) ? $desktop_mode : 'cloud'
    ]);
});
