...
class SellController extends ApiController
{
    ...
    public function index()
    {
        ...
        $has_mobile_access = $module_util->hasThePermissionInSubscription($business_id, 'hms_mobile_app_access') || $user->can('hms_mobile_app_access') || $user->can('superadmin');

        if (! $is_admin && ! $has_mobile_access && ! auth()->user()->hasAnyPermission(['sell.view', 'direct_sell.access', 'direct_sell.view', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
        ...
        $with = ['sell_lines', 'payment_lines', 'contact', 'hms_booking_lines', 'hms_booking_extras.extra'];
        ...
        if (! $is_admin && ! $has_mobile_access && ! $user->can('direct_sell.view')) {
