...
function __print_receipt(section_id = null) {
    // Ensure screen stays locked during printing
    var ensureScreenLocked = function() {
        // Trigger window resize to activate any screen lock handlers
        window.dispatchEvent(new Event('resize'));
        
        // Reset any print-specific styles
        setTimeout(function() {
            if ($('.main-sidebar').length) {
                $('.main-sidebar, .left-side, .main-header').show();
            }
            ...
            // Clear receipt section after printing
            $('#receipt_section').html('');
        }, 100);
    };

    // Add afterprint event listener to maintain screen lock
    window.addEventListener('afterprint', ensureScreenLocked, false);
