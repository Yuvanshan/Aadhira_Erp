...
function getPageLoaderMessage() {
    if (typeof LANG !== 'undefined' && LANG.loading) {
        return LANG.loading + '...';
    }
...
function showPageLoader(message) {
    var loader = $('#page_loading_overlay');
    if (!loader.length) {
        return;
    }

    loader.find('[data-page-loader-message]').text(message || getPageLoaderMessage());
    loader.addClass('is-visible').attr('aria-hidden', 'false');
    $('body').addClass('page-loading-active');
}
...
function hidePageLoader() {
    ...
    $('body').removeClass('page-loading-active');
}

function canShowPageLoaderForLink(link) {
    ...
    if (
        ...
        typeof link.attr('download') !== 'undefined' ||
        link.hasClass('btn-modal') ||
        link.hasClass('btn-modal-coupon') ||
        link.hasClass('btn-modal-extra') ||
        link.hasClass('btn-modal-checkIn') ||
        link.hasClass('btn-modal-unavailable') ||
        (link.attr('class') && link.attr('class').indexOf('btn-modal') !== -1) ||
        link.hasClass('dropdown-toggle') ||
        link.hasClass('print-invoice-link') ||
        link.hasClass('delete-sale') ||
        link.hasClass('link_confirmation') ||
        link.data('toggle') ||
        link.data('target') ||
        link.data('href') ||
        (link.attr('class') && (
            link.attr('class').indexOf('delete') !== -1 ||
            link.attr('class').indexOf('destroy') !== -1 ||
            link.attr('class').indexOf('remove') !== -1
        )) ||
        href.indexOf('delete') !== -1 ||
        href.indexOf('destroy') !== -1 ||
...
$(document).ready(function () {
    ...
    // Force close page loading overlay on Shift + Escape key press
    ...
    $(document).on('click', 'a', function (e) {
        ...
        var link = $(this);
        if (canShowPageLoaderForLink(link)) {
            showPageLoader();
        }
    });
});
